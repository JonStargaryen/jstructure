...
public class A05_ComputeTMAlignStatistics {
    public static void main(String[] args) {
        String output = Start2FoldConstants.walk(Start2FoldConstants.START2FOLD_DIRECTORY.resolve("tmalign"))
        ...
        Start2FoldConstants.write(Start2FoldConstants.START2FOLD_DIRECTORY.resolve("reconstruction-performance.csv"),
                output);
    }
    ...
    private static Optional<String> handleResultFile(Path tmalignFile) {
        ...
        if(strategy.equals("early") || strategy.equals("full") || strategy.equals("deleted") || strategy.equals("late")) {
            contactMapPath = Start2FoldConstants.START2FOLD_DIRECTORY.resolve("maps")
        ...
        try(Stream<String> stream = Start2FoldConstants.lines(contactMapPath)) {
            numberOfContacts = (int) stream.count() - 1;
        }
        int numberOfResidues;
        try(Stream<String> stream = Start2FoldConstants.lines(Start2FoldConstants.START2FOLD_DIRECTORY.resolve("maps").resolve(pdbId + "_A.fasta"))) {
        ...
        try {
            Start2FoldConstants.TMAlignResult tmAlignResult = Start2FoldConstants.parseTMAlignResultFile(tmalignFile);
