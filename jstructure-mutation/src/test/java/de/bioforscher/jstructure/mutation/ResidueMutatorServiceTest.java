...
public class ResidueMutatorServiceTest {
    ...
    public void shouldIntroduceMutation() {
        ...
        Protein mutatedProtein = new ResidueMutatorService().mutate(protein, chainId, position, target);
        ...
        Assert.assertEquals("atom count does not match", target.getGroupPrototype().getPrototypeAtoms().stream().map(Atom::getElement).filter(Element::isHeavyAtom).count() - 1, mutatedGroup.getAtoms().size());
