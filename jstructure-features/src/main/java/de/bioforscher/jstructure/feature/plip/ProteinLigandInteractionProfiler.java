...
public class ProteinLigandInteractionProfiler extends ExternalLocalService {
    ...
    public InteractionContainer fetch(Structure structure) {
        List<Interaction> mergedInteractions = fetchLigandInteractions(structure).getInteractions();
        mergedInteractions.addAll(fetchPolymerInteractions(structure).getInteractions());
        return new InteractionContainer(mergedInteractions);
    }
    ...
    public InteractionContainer fetchPolymerInteractions(Structure structure) {
        return new InteractionContainer(structure.chains()
                .map(chain -> {
                    try {
                        String pdbId = structure.getProteinIdentifier().getPdbId();
                        String chainId = chain.getChainIdentifier().getChainId();
                        String boundary = Long.toHexString(System.currentTimeMillis());
                        URLConnection connection = (new URL("https://biosciences.hs-mittweida.de/plip/api/provided/intra/" + pdbId + "/" + chainId)).openConnection();
    ...
    public InteractionContainer parseDocument(Structure structure, Document document) {
