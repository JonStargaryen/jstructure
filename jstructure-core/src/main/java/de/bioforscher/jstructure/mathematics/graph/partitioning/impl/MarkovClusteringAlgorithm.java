...
import de.bioforscher.jstructure.mathematics.graph.partitioning.GraphPartitioningAlgorithm;
 ...
 */
public class MarkovClusteringAlgorithm implements GraphPartitioningAlgorithm {
    ...
    public MarkovClusteringAlgorithm(double expandFactor, double inflateFactor, double multFactor, double maxIterations, ToDoubleFunction<Edge> similarityFunction) {
    ...
    public MarkovClusteringAlgorithm() {
    ...
    private <N> PartitionedGraph<N> assign(double[] M, int n, Graph<N> graph) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if(!cluster.isEmpty()) {
                clusters.add(new Module<>(String.valueOf(clusters.size() + 1), graph, cluster));
            }
        }
